...
def configure_tpu(FLAGS):
  if FLAGS.use_tpu:
    tpu_cluster = tf2.compat.v1.estimator.cluster_resolver.TPUClusterResolver(
  ...
  run_config = tf.estimator.tpu.RunConfig(
      ...
      tpu_config=tf2.compat.v1.estimator.tpu.TPUConfig(
...
def get_train_op(FLAGS, total_loss, grads_and_vars=None):
  ...
  if FLAGS.use_tpu:
    optimizer = tf2.compat.v1.estimator.tpu.CrossShardOptimizer(optimizer)
...
def clean_ckpt(_):
  ...
  var_list = tf2.compat.v1.estimator.framework.list_variables(input_ckpt)
  ...
  reader = tf2.compat.v1.estimator.framework.load_checkpoint(input_ckpt)
...
def avg_checkpoints(model_dir, output_model_dir, last_k):
  ...
  var_list = tf2.compat.v1.estimator.framework.list_variables(checkpoints[0])
  var_values, var_dtypes = {}, {}
  for (name, shape) in var_list:
    if not name.startswith("global_step"):
      var_values[name] = np.zeros(shape)
  for checkpoint in checkpoints:
    reader = tf2.compat.v1.estimator.framework.load_checkpoint(checkpoint)
    for name in var_values:
      tensor = reader.get_tensor(name)
      var_dtypes[name] = tensor.dtype
      var_values[name] += tensor
