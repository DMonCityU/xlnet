...
def configure_tpu(FLAGS):
  if FLAGS.use_tpu:
    tpu_cluster = tf.compat.v1.estimator.cluster_resolver.TPUClusterResolver(
...
def clean_ckpt(_):
  ...
  var_list = tf.compat.v1.estimator.framework.list_variables(input_ckpt)
  ...
  reader = tf.compat.v1.estimator.framework.load_checkpoint(input_ckpt)
...
def avg_checkpoints(model_dir, output_model_dir, last_k):
  ...
  var_list = tf.compat.v1.estimator.framework.list_variables(checkpoints[0])
  var_values, var_dtypes = {}, {}
  for (name, shape) in var_list:
    if not name.startswith("global_step"):
      var_values[name] = np.zeros(shape)
  for checkpoint in checkpoints:
    reader = tf.compat.v1.estimator.framework.load_checkpoint(checkpoint)
    for name in var_values:
      tensor = reader.get_tensor(name)
      var_dtypes[name] = tensor.dtype
      var_values[name] += tensor
