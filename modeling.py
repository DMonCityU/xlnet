...
def post_attention(h, attn_vec, d_model, n_head, d_head, dropout, is_training,
  ...
  if residual:
    output = tf.compat.v1.estimator.layers.layer_norm(attn_out + h, begin_norm_axis=-1,
                                          scope='LayerNorm')
  else:
    output = tf.compat.v1.estimator.layers.layer_norm(attn_out, begin_norm_axis=-1,
                                          scope='LayerNorm')
